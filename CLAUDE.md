...
| `assets/js/terminal.js` | xterm.js 5.5 + WebSocket buffer + URI decode + multi-tab support |
| `assets/js/i18n.js` | EN/ZH messages, setLocale/toggle |
| `assets/js/preview.js` | Web Preview panel, embeds URL via iframe, toolbar + workspace bar toggle |
...
| `openUrl` | `{url}` | `{ok, url}` | Opens URL in device browser via termux-open-url |
